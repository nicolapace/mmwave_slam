...
public:
   ...
   //void DataUARTHandler(ros::NodeHandle* nh) : currentBufp(&pingPongBuffers[0]) , nextBufp(&pingPongBuffers[1]) {}
    ...
    /*User callable function to set setUseAgglo*/
    void setPuntiDesiderati(int mypunti_desiderati);

    /*User callable function to set setUseAgglo*/
    void setUseAgglo(bool myuseAgglo);

    /*User callable function to set setUseLior*/
    void setUseLior(bool myuseLior);

    /*User callable function to set setUseLior*/
    void settakeSideInfo(bool mytakeSideInfo);

    void setdistInlierTh(double mydist_outlier_th);

    void setcdist_th(double mycdist_th);
...
private:
    ...
    int punti_desiderati;

    bool USE_AGGLOMERATIVE;
    bool USE_LIOR_FILTER;
    bool TAKE_SIDEINFO;

    double dist_outlier_th;
    double cdist_th;
